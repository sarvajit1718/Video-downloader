...
def _compact_info_for_cache(info: Dict[str, Any]) -> Dict[str, Any]:
    """
    Produce a very small cache-friendly dict with only:
      - top-level metadata: _type, id, title, uploader, duration (seconds), upload_date (YYYY-MM-DD),
        view_count, like_count, thumbnail
      - audio: best m4a and best opus (format_id, ext, filesize)
      - video: for MAIN_HEIGHTS, for mp4 and webm store { format_id, ext, filesize, resolution, paired_audio_format_id }
    """
    comp: Dict[str, Any] = {}

    # --- top-level fields (minimal) ---
    for k in ("_type", "id", "title", "uploader", "view_count", "like_count", "thumbnail"):
        if k in info:
            comp[k] = info[k]
    ...
    # choose preferred audio for pairing (prefer m4a if present)
    preferred_audio_for_pairing = None
    if best_m4a:
        preferred_audio_for_pairing = best_m4a
    elif best_opus:
        preferred_audio_for_pairing = best_opus

    # --- video: for each MAIN_HEIGHT keep mp4 + webm minimal info ---
    comp["video"] = {}
    for h in MAIN_HEIGHTS:
        key = f"{h}p"
        comp["video"][key] = {"mp4": None, "webm": None}
        ...
        for container in ("mp4", "webm"):
            candidates = [
                ...
                and (f.get("ext") or "").lower() == container
                and (_filesize_of(f) or f.get("tbr") or f.get("fps"))
            ]

            if not candidates:
                comp["video"][key][container] = None
                continue

            # score using existing scoring helper; prefer mp4/webm ordering here is irrelevant
            chosen = max(candidates, key=lambda x: _score_video_candidate(x, ["mp4", "webm"]))

            comp["video"][key][container] = {
                "format_id": chosen.get("format_id"),
                "ext": chosen.get("ext"),
                "filesize": _filesize_of(chosen),
                "resolution": chosen.get("resolution") or f"{h}p",
                "paired_audio_format_id": preferred_audio_for_pairing.get("format_id") if preferred_audio_for_pairing else None
            }

    # remove empty resolution entries where both containers are None
    empty_keys = [k for k, v in list(comp["video"].items()) if v.get("mp4") is None and v.get("webm") is None]
    for k in empty_keys:
        del comp["video"][k]

    # cache timestamp (UTC)
    comp["_cached_at"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
...
def _expand_compact_if_needed(compact_info: Dict[str, Any]) -> Dict[str, Any]:
    """Return the compact_info as-is. Kept as a named function to make future expansion trivial."""
    return compact_info
...
class MetadataExtractor:
    ...
    def extract_raw(self, url: str, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if self._cache_raw and not force_refresh:
            if url in self._raw_cache:
                # return compacted form (which contains the formats the rest of code needs)
                return _expand_compact_if_needed(self._raw_cache[url])
    ...
    def extract_and_clean(self, url: str, mode: str = "video", force_refresh: bool = False) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        # if the cache returned compact info we still have 'formats' and other fields needed
